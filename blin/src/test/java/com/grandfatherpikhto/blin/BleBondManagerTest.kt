...
class BleBondManagerTest {
    ...
    fun bondDevice() = runTest(dispatcher) {
        ...
        assertEquals(BleBondState(bluetoothDevice,
            AbstractBleBondManager.State.Request), bleBondManager.bondState)
        ...
        assertEquals(BleBondState(bluetoothDevice, AbstractBleBondManager.State.Bonded),
    ...
    fun errorBondingDevice() = runTest(UnconfinedTestDispatcher()) {
        ...
        ShadowLooper.shadowMainLooper().idle()
        assertEquals(BleBondState(bluetoothDevice,
            AbstractBleBondManager.State.Error), bleBondManager.bondState)
    }
    ...
    fun rejectBondDevice() = runTest(UnconfinedTestDispatcher()) {
        ...
        ShadowLooper.shadowMainLooper().idle()
        assertEquals(BleBondState(bluetoothDevice, AbstractBleBondManager.State.Reject),
            bleBondManager.bondState)
    }
}